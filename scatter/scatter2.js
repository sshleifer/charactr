...
(function() {
  var addXax, center, dispatch, exp, height, margin, svg, tabulate, tabwidth, tool2, tooltip, width, xAxis, xMap, xScale, xValue, yAxis, yMap, yScale, yValue,
  ...
  dispatch.on("load.menu", function(data) {
    var selectbox;
    selectbox = d3.select('.page').append('div').attr({
  ...
  dispatch.on("load.scatter", function(data) {
    ...
    dots = svg.selectAll('.dot').data(data).enter().append('circle').attr({
      'id': (function(d) {
        return d.cname;
      }),
      'class': 'dot',
